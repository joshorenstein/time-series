...
b <- b %>% arrange(Year) %>% filter(Year>1889)
b
...
ggplot(b,aes(x=Year,y=Attendance)) + geom_line()
#check ACF plot
ggAcf(b$Attendance,type="correlation")
#run ADF test
ggPacf(b$Attendance,type="correlation")
##Transforming for Stationarity and Identifying Model Parameters
#fit AR model
ar.model.b  <- auto.arima(b$Attendance,max.d=0,max.q=0,allowdrift=T)
ar.model.b
#fit MA model
ma.model.b  <- auto.arima(b$Attendance,max.d=0,max.q=0,allowdrift=T)
ma.model.b

#fir ARMA model
arma.model.b  <- auto.arima(b$Attendance,max.d=0,allowdrift=T)
arma.model.b


#fit ARIMA model
arima.model.b <- auto.arima(b$Attendance,allowdrift=T)
arima.model.b
